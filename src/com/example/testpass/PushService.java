...
public class PushService extends Service {
	...
	private synchronized void start() {
		...
		if (mConnection != null) {
			try {
				mConnection.publishToTopic("test/topic", "aaaaaaaaaaa");
	...
	private synchronized void keepAlive() {
		try {
			...
			System.out.println("In keep alive");
			if (mStarted == true && mConnection != null) {
				System.out.println("reconnect: " + mStarted + " | "
						+ mConnection);
	...
	private void messageBack(String msgID) {
		this.waitForCallBack.remove(msgID);
	}
	...
	private class CallBackReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (!intent.getStringExtra("msg").trim().toLowerCase()
					.equals("added")) {
				try {
					mConnection.publishToTopic("test/topic",
			...
			// System.out.println("Get text to send: " +
			// intent.getStringExtra("msg"));
			// System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
			// + intent.getStringExtra("msg"));
			// refreshText(intent.getStringExtra("msg"));
		}
	}
}